...
namespace ag
{
	namespace obsolete
	{
		...
		StaticSolver::SolverResult StaticSolver::check_draw_in_1(ActionList &actions)
		{
			...
			{
				if (is_anything_forbidden_for (get_own_sign())){ // only if there can be any forbidden move, so in renju and for cross (black) player
		...
		Score StaticSolver::try_solve_fork_4x3(Move move)
		{
			...
			if (is_anything_forbidden_for (get_own_sign()))return Score(); // there is a possibility that inside the three there will later be a forbidden move -> too complicated to check statically
