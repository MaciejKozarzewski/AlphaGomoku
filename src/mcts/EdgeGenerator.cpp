...
namespace
{
	...
	{
		assert(task.getPriorEdges().size() > 0);
...
namespace ag
{
	...
	SolverGenerator::SolverGenerator(int maxEdges) :
			max_edges(maxEdges)
	{
	}
	std::unique_ptr<EdgeGenerator> SolverGenerator::clone() const
	{
		return std::make_unique<SolverGenerator>(max_edges);
	}
	void SolverGenerator::generate(SearchTask &task) const
	{
		if (task.mustDefend() or task.isReadySolver())
			create_moves_from_solver(task); // we have some prior moves generated by the solver, use them
		else
			create_moves_from_policy(task, 0.0f); // no moves were generated in the solver, use policy to create them
	...
	void SequentialHalvingGenerator::generate(SearchTask &task) const
	{
		if (task.getRelativeDepth() == 0)
		{
			...
			generate_at_root(task);
		}
		else
			generate_below_root(task);
		assert(task.getEdges().size() > 0);
	}
	/*
	 * private
	 */
	void SequentialHalvingGenerator::generate_at_root(SearchTask &task) const
	{
		if (task.mustDefend() or task.isReadySolver())
		{
			...
			for (auto edge = task.getPriorEdges().begin(); edge < task.getPriorEdges().end(); edge++)
			{
				...
				switch (edge->getProvenValue())
				{
					case ProvenValue::UNKNOWN:
						break;
					case ProvenValue::LOSS:
						task.getPolicy().at(move.row, move.col) = 0.0f;
						task.getActionValues().at(move.row, move.col) = Value(0.0f, 0.0f, 1.0f);
						break;
					case ProvenValue::DRAW:
						task.getActionValues().at(move.row, move.col) = Value(0.0f, 1.0f, 0.0f);
						break;
					case ProvenValue::WIN:
						task.getPolicy().at(move.row, move.col) = 1.0e3f;
						task.getActionValues().at(move.row, move.col) = Value(1.0f, 0.0f, 0.0f);
						break;
				}
			}
		}
	...
	void SequentialHalvingGenerator::generate_below_root(SearchTask &task) const
	{
		SolverGenerator(max_edges).generate(task);
	}
