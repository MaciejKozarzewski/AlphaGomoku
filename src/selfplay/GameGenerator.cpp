...
namespace ag
{
	...
	void GameGenerator::load(const Json &json, const SerializedObject &binary_data)
	{
		...
		game_data_storage = GameDataStorage(binary_data, offset, 201);
...
//		data.print();
	...
	void GameGenerator::prepare_search()
	{
		...
		tree.setEdgeGenerator(
				UnifiedGenerator(mcts_config.max_children, mcts_config.policy_expansion_threshold, mcts_config.policy_temperature, true));
	}
