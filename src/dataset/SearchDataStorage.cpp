...
namespace ag
{
	...
	SearchDataStorage_v201::SearchDataStorage_v201(const SerializedObject &binary_data, size_t &offset)
	{
		value_scale = fp16_format::to_fp32(binary_data.load<uint16_t>(offset));
		offset += sizeof(uint16_t);
		policy_scale = fp16_format::to_fp32(binary_data.load<uint16_t>(offset));
		...
		flags = BitMask1D<uint16_t>(binary_data.load<uint16_t>(offset));
		offset += sizeof(uint16_t);

		unserializeVector(storage, binary_data, offset);
	}
	int SearchDataStorage_v201::numberOfEntries() const noexcept
	{
		return storage.size();
	}
	int SearchDataStorage_v201::getMoveNumber() const noexcept
	{
		return move_number;
	}
	void SearchDataStorage_v201::loadFrom(const SearchDataPack &pack)
	{
		move_number = 0;
		size_t entries_count = 0;
		policy_scale = 0.0f;
	...
	void SearchDataStorage_v201::storeTo(SearchDataPack &pack) const
	{
		int current_idx = 0;
		float win_rate = 0.0f, draw_rate = 0.0f;
		int sum_visits = 0;
	...
	void SearchDataStorage_v201::serialize(SerializedObject &binary_data) const
	{
		binary_data.save(static_cast<uint16_t>(fp16_format::to_lowp(value_scale)));
		binary_data.save(static_cast<uint16_t>(fp16_format::to_lowp(policy_scale)));
		binary_data.save(static_cast<uint16_t>(fp16_format::to_lowp(visit_scale)));
		binary_data.save(minimax_score);
		binary_data.save(move_number);
		binary_data.save(flags.raw());
		serializeVector(storage, binary_data);
	}
	void SearchDataStorage_v201::print() const
	{
		std::cout << "move number = " << move_number << '\n';
		std::cout << "value scale = " << value_scale << '\n';
		std::cout << "policy scale = " << policy_scale << '\n';
