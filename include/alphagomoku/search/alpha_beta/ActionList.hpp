...
//	};
	...
	class ActionStack
	{
		...
		public:
			...
			void increment(size_t num = 1) noexcept
			{
				assert(m_offset + num < size());
				m_offset += num;
				m_max_offset = std::max(m_max_offset, m_offset);
			}
			void decrement(size_t num = 1) noexcept
			{
				assert(0 <= m_offset - num);
			...
			size_t max_offset() const noexcept
			{
				return m_max_offset;
			}
			Action& operator[](size_t index)
	...
	class ActionList
	{
		...
		public:
			...
			ActionList(ActionStack &stack) noexcept :
					m_stack(&stack)
			{
				stack.reset();
				m_children = stack.back();
			}
			ActionList(ActionStack &stack, ActionList &parent, int moveIndex) noexcept :
					m_stack(&stack),
					m_children(stack.back()),
					m_distance_from_root(parent.m_distance_from_root + 1),
					last_move(parent[moveIndex].move)
			{
			}
			~ActionList() noexcept
			{
				m_stack->decrement(m_size);
			}
