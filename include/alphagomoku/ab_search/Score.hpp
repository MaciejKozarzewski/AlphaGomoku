...
#ifndef ALPHAGOMOKU_AB_SEARCH_SCORE_HPP_
#define ALPHAGOMOKU_AB_SEARCH_SCORE_HPP_
...
namespace ag
{
	enum class ProvenScore
	{
		...
		NOT_WIN, /* the position was proven to not contain any winning moves */
		NOT_LOSS, /* the position was proven to have at least one non-losing move */
	...
	inline ProvenScore invertProvenScore(ProvenScore ps) noexcept
	{
		switch (ps)
		{
			...
			case ProvenScore::NOT_WIN:
				return ProvenScore::NOT_LOSS;
			case ProvenScore::NOT_LOSS:
				return ProvenScore::NOT_WIN;
	 ...
	 *
	 * must_defend == false
	 *   score starts from DISPROVED	(even if all moves are losing it's not a loss because but we didn't have to defend. Worst outcome we can have here is simply no proof)
	 *
	 *   Possible outcomes:
	 *        max_score == LOSS      ->  N/A
	 *        max_score == DRAW      ->  proven score = DRAW
	 * DRAW < max_score <  WIN       ->  proven score = UNKNOWN		(no proved win and at least one move is not proven at all)
	...
	class Score
	{
		...
		public:
			...
			static Score not_loss() noexcept
			{
				return Score(ProvenScore::NOT_LOSS);
			}
			static Score not_win() noexcept
			{
				return Score(ProvenScore::NOT_WIN);
			}
...
#endif /* ALPHAGOMOKU_AB_SEARCH_SCORE_HPP_ */
