...
#include <alphagomoku/tss/FastHashFunction.hpp>
	...
	namespace tss
	{
		...
		class SharedTableData
		{
			...
			public:
				...
				{
					return HashKey<64>(m_data & mask); // extract 16 highest bits
				}
		};
		template<int N>
		class SharedHashTable
		{
				class Entry
				{
						static constexpr HashKey<64> mask = 0xFFFF000000000000ull;
				...
				FastHashFunction<128> m_hash_function;
				HashKey<64> m_mask;
			...
			public:
				...
				SharedTableData seek(const HashKey<128> &hash) const noexcept
				{
